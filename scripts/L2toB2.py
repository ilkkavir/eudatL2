...
if __name__ == '__main__':
        ...
        ## Get configuration
        config=ConfigParser.SafeConfigParser({'baseURI':'eiscat-raid://localhost/', 'dbHost' : 'localhost', 'dbUser' : 'archiver', 'dbPass' : '', 'db' : 'disk_archive', 'maxproc' : 1 }) #defaults
        ...
        with catalogquery.CatalogQuery(config) as DBq:
                ...
                for eid in eids:
                        ...
                        for eloc in elocs:
                                ...
                                # assert(len(iid)==1), "Ambiguous: none or more than one info dir found"
                                try:
                                        iid=iid[0]
                                        iloc=DBq.get_locs(iid['resource_id'])
                                        # assert (len(iloc)==1), "Ambiguous: more than one info dir found"
                                        iloc=iloc[0]
                                except:
                                        iloc={'location': ''}
